# Mock Exam Tracker

> Target: 7.5–8.0 overall | Minimum: 6.5 each section

---

## Score History

| Mock | Date | Listening | Reading | Writing | Speaking | **Overall** |
|---|---|---|---|---|---|---|
| Diagnostic (Wk 2) | | | | | | |
| Mock #1 (Wk 15) | | | | | | |
| Mock #2 (Wk 17) | | | | | | |
| Mock #3 (Wk 19) | | | | | | |
| **Real Exam** | | | | | | **TARGET: 7.5+** |

---

## 📊 LÍNEA BASE DE WRITING — Task 2 completo, 15 de agosto de 2026

**Prompt 1** (salud pública / instalaciones deportivas) · *Discuss both views and give your own opinion* · 40 min cronometrados.

| Criterio | Banda |
|---|---|
| Task Achievement | 5.5 |
| Coherence & Cohesion | 6.0 |
| Lexical Resource | 6.0 |
| Grammatical Range & Accuracy | 5.5 |
| **Global** | **~5.5** |

### Reparto del tiempo y longitud

| Bloque | Palabras | Objetivo | |
|---|---|---|---|
| Introducción | 43 | 40–60 | ✅ |
| Body 1 | 53 | 90–110 | ❌ mitad |
| Body 2 | 53 | 90–110 | ❌ mitad |
| Conclusión | 33 | 40–60 | ⚠️ |
| **TOTAL** | **182** | **250 mín.** | ❌ |

**Tiempo:** 15 min planificando (objetivo: 5–8) · resto escribiendo y corrigiendo. **Los 40 minutos se usaron completos por primera vez.**

### La causa raíz — no fue el inglés

Las notas de la autopsia del viernes registraron, para el Body 1: *"Two robust sentences are enough"*. Se escribieron dos oraciones y salieron 53 palabras.

**El Body 1 del ensayo modelo tiene cuatro oraciones**, no dos — Claim → Explain → Example → Link, ~90 palabras. Las oraciones del modelo son largas y se contaron mal.

> **Corrección permanente: cada body son 4 oraciones.** Solo con eso el ensayo pasa de 182 a ~270 palabras sin mejorar el inglés.

### Lo que sí funcionó

- ✅ **40 minutos completos** — patrón de seis tareas entregando temprano, roto
- ✅ **Planificación previa** — primera vez en toda la preparación
- ✅ **13 de 18 palabras del batch activadas**
- ✅ Los cuatro bloques en orden, con conectores correctos (*While · Consequently · Nevertheless · In conclusion*)
- ✅ **Opinión propia explícita** en la conclusión — requisito del *Discuss both views*
- ✅ Autodiagnóstico acertado: subrayó *positively* y *negatively* (ambas mal) y dudó de *equally* (bien)

### Reparación del 17 de agosto — el mismo ensayo, completado

| Bloque | 15-ago | 17-ago | Objetivo |
|---|---|---|---|
| Introducción | 43 | 47 | 40–60 ✅ |
| Body 1 | 53 | **86** | 90–110 ✅ |
| Body 2 | 53 | **76** | 90–110 ✅ |
| Conclusión | 33 | 39 | 40–60 ⚠️ |
| **TOTAL** | **182** | **248** | **250 mín.** ❌ *(faltan 2)* |

**Bandas tras la reparación: ~6** (TA 6 · CC 6 · LR 6.5 · GR 6), desde 5.5.

**Los bodies se completaron sin escribir nada nuevo**, solo añadiendo los movimientos que les faltaban: Body 1 de 53 a 86 por cuenta propia, Body 2 de 53 a 76 con la oración LINK. Confirma el diagnóstico del sábado: **no era el inglés, era la plantilla.**

⚠️ **El ensayo sigue 2 palabras bajo el mínimo.** Un cálculo previo lo dio en ~252, pero era una estimación aritmética hecha sin ver el texto final; al verificarlo son **248**. La razón: las correcciones de artículos (*the sleep quality* → *sleep quality*, *a total wellbeing* → *total wellbeing*) también **restan** palabras.

> **Lección: verificar el conteo sobre el texto, no estimarlo.** Y no apuntar a 251 — un artículo que se quite vuelve a bajar del mínimo. **Apuntar a 265–280.**

> ⚠️ Esta cifra **no es comparable** con la línea base: es reparación sin cronómetro y con la lista de errores delante. La medición real en condiciones de examen es el **sábado 22**.

### Una cosa que corregir antes del sábado 22

**4 oraciones por body.** Nada más. Es el cambio de mayor impacto por unidad de esfuerzo de toda la preparación hasta ahora.

> ⚠️ **El sábado 22 (prompt 4 — brecha ricos/pobres) es la primera banda fría de verdad:** tipo *Problem–Solution*, material nunca trabajado. Anotar las dos cifras por separado y **no compararlas** — son pruebas distintas.

---

## Band Score Reference

| Raw Score (Listening/Reading out of 40) | Band |
|---|---|
| 39–40 | 9.0 |
| 37–38 | 8.5 |
| 35–36 | 8.0 |
| 32–34 | 7.5 |
| 30–31 | 7.0 |
| 26–29 | 6.5 |
| 23–25 | 6.0 |
| 18–22 | 5.5 |

---

## Mock #1 Analysis (Week 15)

**Date:** ___________  
**Scores:** L: ___ / R: ___ / W: ___ / S: ___ / **Overall: ___**

### Listening
- Sections where I lost most marks: 
- Question types that caused errors:
- One specific fix before Mock #2:

### Reading
- Passages where I lost most marks:
- Question types that caused errors:
- One specific fix before Mock #2:

### Writing
- Task 1 main weakness:
- Task 2 main weakness:
- One specific fix before Mock #2:

### Speaking
- Part where fluency broke down:
- Vocabulary gaps noticed:
- One specific fix before Mock #2:

---

## Mock #2 Analysis (Week 17)

**Date:** ___________  
**Scores:** L: ___ / R: ___ / W: ___ / S: ___ / **Overall: ___**

### What improved since Mock #1?

### What still needs work?

### One priority for the final 3 weeks:

---

## Mock #3 Analysis (Week 19)

**Date:** ___________  
**Scores:** L: ___ / R: ___ / W: ___ / S: ___ / **Overall: ___**

### Am I on track for 7.5?

### Any last adjustments before exam day?

---

## Exam Day Notes

**Test center:** ___________  
**Date & time:** ___________  
**ID to bring:** ___________  
**Speaking test time (often separate day):** ___________
